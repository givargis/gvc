// Copyright (c) Tony Givargis, 2024-2026

#include "g_ec.h"

static uint8_t G_H[256][16];
static uint8_t G_L[256][16];
static uint8_t A_H[256][16];
static uint8_t A_L[256][16];
static uint8_t B_H[256][256][16];
static uint8_t B_L[256][256][16];

#define U64(x) ( (uint64_t)(x) )

static uint8_t
mul(uint8_t a, uint8_t b)
{
	uint8_t p;

	p = 0;
	while (a && b) {
		if (b & 1) {
			p ^= a;
		}
		if (a & 0x80) {
			a = (a << 1) ^ 0x11d;
		}
		else {
			a <<= 1;
		}
		b >>= 1;
	}
	return p;
}

static uint8_t
power(uint8_t a, int b)
{
	uint8_t p;

	p = 1;
	b %= 255;
	if (b < 0) {
		b += 255;
	}
	while (b) {
		if (b & 1) {
			p = mul(p, a);
		}
		a = mul(a, a);
		b >>= 1;
	}
	return p;
}

void
g_ec_init(void)
{
	uint8_t g, a, b;

	/**
	 * G(j) : {02}^j
	 */

	for (int j=0; j<256; ++j) {
		g = power(2, j);
		for (int i=0; i<16; ++i) {
			G_H[j][i] = mul(g, (uint8_t)(i << 4));
			G_L[j][i] = mul(g, (uint8_t)(i & 15));
		}
	}

	/**
	 * A'(x,y) : A(y - x) | y > x
	 * A(j)    : {02}^j * ( {02}^j + {01} )^-1
	 */

	for (int j=1; j<256; ++j) {
		g = power(2, j);
		a = mul(g, power(g^1, 254));
		for (int i=0; i<16; ++i) {
			A_H[j][i] = mul(a, (uint8_t)(i << 4));
			A_L[j][i] = mul(a, (uint8_t)(i & 15));
		}
	}

	/**
	 * B(x,y) : {02}^-x * ( {02}^(y-x) + {01} )^-1 | y > x
	 */

	for (int x=0; x<256; ++x) {
		for (int y=x+1; y<256; ++y) {
			g = power(2, y - x);
			b = mul(power(2, 255 - x), power(g^1, 254));
			for (int i=0; i<16; ++i) {
				B_H[x][y][i] = mul(b, (uint8_t)(i << 4));
				B_L[x][y][i] = mul(b, (uint8_t)(i & 15));
			}
		}
	}
}

void
g_ec_encode_pq(void *buf, int k, int n)
{
	uint64_t d, *p, *q;

	assert( buf );
	assert( 0 == (n % 8) );
	assert( (G_EC_MIN_K <= k) && (G_EC_MAX_K >= k) );
	assert( (G_EC_MIN_N <= n) && (G_EC_MAX_N >= n) );

	p = G_EC_P(buf, k, n);
	q = G_EC_Q(buf, k, n);
	memset(p, 0, n);
	memset(q, 0, n);
	for (int j=0; j<k; ++j) {
		for (int i=0; i<(n/8); ++i) {
			d = G_EC_D(buf, j, n)[i];
			p[i] ^= d;
			q[i] ^= ((U64(G_H[j][d >> 60 & 15]) << 56 |
				  U64(G_H[j][d >> 52 & 15]) << 48 |
				  U64(G_H[j][d >> 44 & 15]) << 40 |
				  U64(G_H[j][d >> 36 & 15]) << 32 |
				  U64(G_H[j][d >> 28 & 15]) << 24 |
				  U64(G_H[j][d >> 20 & 15]) << 16 |
				  U64(G_H[j][d >> 12 & 15]) <<  8 |
				  U64(G_H[j][d >>  4 & 15])) ^
				 (U64(G_L[j][d >> 56 & 15]) << 56 |
				  U64(G_L[j][d >> 48 & 15]) << 48 |
				  U64(G_L[j][d >> 40 & 15]) << 40 |
				  U64(G_L[j][d >> 32 & 15]) << 32 |
				  U64(G_L[j][d >> 24 & 15]) << 24 |
				  U64(G_L[j][d >> 16 & 15]) << 16 |
				  U64(G_L[j][d >>  8 & 15]) <<  8 |
				  U64(G_L[j][d >>  0 & 15])));
		}
	}
}

void
g_ec_encode_p(void *buf, int k, int n)
{
	uint64_t *p;

	assert( buf );
	assert( 0 == (n % 8) );
	assert( (G_EC_MIN_K <= k) && (G_EC_MAX_K >= k) );
	assert( (G_EC_MIN_N <= n) && (G_EC_MAX_N >= n) );

	p = G_EC_P(buf, k, n);
	memset(p, 0, n);
	for (int j=0; j<k; ++j) {
		for (int i=0; i<(n/8); ++i) {
			p[i] ^= G_EC_D(buf, j, n)[i];
		}
	}
}

void
g_ec_encode_q(void *buf, int k, int n)
{
	uint64_t d, *q;

	assert( buf );
	assert( 0 == (n % 8) );
	assert( (G_EC_MIN_K <= k) && (G_EC_MAX_K >= k) );
	assert( (G_EC_MIN_N <= n) && (G_EC_MAX_N >= n) );

	q = G_EC_Q(buf, k, n);
	memset(q, 0, n);
	for (int j=0; j<k; ++j) {
		for (int i=0; i<(n/8); ++i) {
			d = G_EC_D(buf, j, n)[i];
			q[i] ^= ((U64(G_H[j][d >> 60 & 15]) << 56 |
				  U64(G_H[j][d >> 52 & 15]) << 48 |
				  U64(G_H[j][d >> 44 & 15]) << 40 |
				  U64(G_H[j][d >> 36 & 15]) << 32 |
				  U64(G_H[j][d >> 28 & 15]) << 24 |
				  U64(G_H[j][d >> 20 & 15]) << 16 |
				  U64(G_H[j][d >> 12 & 15]) <<  8 |
				  U64(G_H[j][d >>  4 & 15])) ^
				 (U64(G_L[j][d >> 56 & 15]) << 56 |
				  U64(G_L[j][d >> 48 & 15]) << 48 |
				  U64(G_L[j][d >> 40 & 15]) << 40 |
				  U64(G_L[j][d >> 32 & 15]) << 32 |
				  U64(G_L[j][d >> 24 & 15]) << 24 |
				  U64(G_L[j][d >> 16 & 15]) << 16 |
				  U64(G_L[j][d >>  8 & 15]) <<  8 |
				  U64(G_L[j][d >>  0 & 15])));
		}
	}
}

void
g_ec_encode_dp(void *buf, int k, int n, int x_)
{
	uint64_t *d, *p;

	assert( buf );
	assert( 0 == (n % 8) );
	assert( (G_EC_MIN_K <= k) && (G_EC_MAX_K >= k) );
	assert( (G_EC_MIN_N <= n) && (G_EC_MAX_N >= n) );
	assert( (0 <= x_) && (k > x_) );

	p = G_EC_P(buf, k, n);
	d = G_EC_D(buf, x_, n);
	memcpy(d, p, n);
	for (int j=0; j<k; ++j) {
		if (j != x_) {
			for (int i=0; i<(n/8); ++i) {
				d[i] ^= G_EC_D(buf, j, n)[i];
			}
		}
	}
}

void
g_ec_encode_dq(void *buf, int k, int n, int x_)
{
	uint64_t d, *q, *x;

	assert( buf );
	assert( 0 == (n % 8) );
	assert( (0 < k) && (256 > k) );
	assert( (0 < n) && (0 == (n % 8)) );
	assert( (G_EC_MIN_K <= k) && (G_EC_MAX_K >= k) );
	assert( (G_EC_MIN_N <= n) && (G_EC_MAX_N >= n) );
	assert( (0 <= x_) && (k > x_) );

	q = G_EC_Q(buf, k, n);
	x = G_EC_D(buf, x_, n);
	memcpy(x, q, n);
	for (int j=0; j<k; ++j) {
		if (j != x_) {
			for (int i=0; i<(n/8); ++i) {
				d = G_EC_D(buf, j, n)[i];
				x[i] ^= ((U64(G_H[j][d >> 60 & 15]) << 56 |
					  U64(G_H[j][d >> 52 & 15]) << 48 |
					  U64(G_H[j][d >> 44 & 15]) << 40 |
					  U64(G_H[j][d >> 36 & 15]) << 32 |
					  U64(G_H[j][d >> 28 & 15]) << 24 |
					  U64(G_H[j][d >> 20 & 15]) << 16 |
					  U64(G_H[j][d >> 12 & 15]) <<  8 |
					  U64(G_H[j][d >>  4 & 15])) ^
					 (U64(G_L[j][d >> 56 & 15]) << 56 |
					  U64(G_L[j][d >> 48 & 15]) << 48 |
					  U64(G_L[j][d >> 40 & 15]) << 40 |
					  U64(G_L[j][d >> 32 & 15]) << 32 |
					  U64(G_L[j][d >> 24 & 15]) << 24 |
					  U64(G_L[j][d >> 16 & 15]) << 16 |
					  U64(G_L[j][d >>  8 & 15]) <<  8 |
					  U64(G_L[j][d >>  0 & 15])));
			}
		}
	}
	x_ = 255 - x_;
	for (int i=0; i<(n/8); ++i) {
		d = x[i];
		x[i] = ((U64(G_H[x_][d >> 60 & 15]) << 56 |
			 U64(G_H[x_][d >> 52 & 15]) << 48 |
			 U64(G_H[x_][d >> 44 & 15]) << 40 |
			 U64(G_H[x_][d >> 36 & 15]) << 32 |
			 U64(G_H[x_][d >> 28 & 15]) << 24 |
			 U64(G_H[x_][d >> 20 & 15]) << 16 |
			 U64(G_H[x_][d >> 12 & 15]) <<  8 |
			 U64(G_H[x_][d >>  4 & 15])) ^
			(U64(G_L[x_][d >> 56 & 15]) << 56 |
			 U64(G_L[x_][d >> 48 & 15]) << 48 |
			 U64(G_L[x_][d >> 40 & 15]) << 40 |
			 U64(G_L[x_][d >> 32 & 15]) << 32 |
			 U64(G_L[x_][d >> 24 & 15]) << 24 |
			 U64(G_L[x_][d >> 16 & 15]) << 16 |
			 U64(G_L[x_][d >>  8 & 15]) <<  8 |
			 U64(G_L[x_][d >>  0 & 15])));
	}
}

void
g_ec_encode_dd(void *buf, int k, int n, int x_, int y_)
{
	uint64_t d, d1, d2, *p, *q, *x, *y;

	assert( buf );
	assert( 0 == (n % 8) );
	assert( (G_EC_MIN_K <= k) && (G_EC_MAX_K >= k) );
	assert( (G_EC_MIN_N <= n) && (G_EC_MAX_N >= n) );
	assert( x_ < y_ );
	assert( (0 <= x_) && (k > x_) );
	assert( (0 <= y_) && (k > y_) );

	p = G_EC_P(buf, k, n);
	q = G_EC_Q(buf, k, n);
	x = G_EC_D(buf, x_, n);
	y = G_EC_D(buf, y_, n);
	memcpy(x, q, n);
	memcpy(y, p, n);
	for (int j=0; j<k; ++j) {
		if ((j != x_) && (j != y_)) {
			for (int i=0; i<(n/8); ++i) {
				d = G_EC_D(buf, j, n)[i];
				y[i] ^= d;
				x[i] ^= ((U64(G_H[j][d >> 60 & 15]) << 56 |
					  U64(G_H[j][d >> 52 & 15]) << 48 |
					  U64(G_H[j][d >> 44 & 15]) << 40 |
					  U64(G_H[j][d >> 36 & 15]) << 32 |
					  U64(G_H[j][d >> 28 & 15]) << 24 |
					  U64(G_H[j][d >> 20 & 15]) << 16 |
					  U64(G_H[j][d >> 12 & 15]) <<  8 |
					  U64(G_H[j][d >>  4 & 15])) ^
					 (U64(G_L[j][d >> 56 & 15]) << 56 |
					  U64(G_L[j][d >> 48 & 15]) << 48 |
					  U64(G_L[j][d >> 40 & 15]) << 40 |
					  U64(G_L[j][d >> 32 & 15]) << 32 |
					  U64(G_L[j][d >> 24 & 15]) << 24 |
					  U64(G_L[j][d >> 16 & 15]) << 16 |
					  U64(G_L[j][d >>  8 & 15]) <<  8 |
					  U64(G_L[j][d >>  0 & 15])));
			}
		}
	}
	for (int i=0; i<(n/8); ++i) {
		d1 = x[i];
		d2 = y[i];
		x[i] = ((U64(A_H[y_ - x_][d2 >> 60 & 15]) << 56 |
			 U64(A_H[y_ - x_][d2 >> 52 & 15]) << 48 |
			 U64(A_H[y_ - x_][d2 >> 44 & 15]) << 40 |
			 U64(A_H[y_ - x_][d2 >> 36 & 15]) << 32 |
			 U64(A_H[y_ - x_][d2 >> 28 & 15]) << 24 |
			 U64(A_H[y_ - x_][d2 >> 20 & 15]) << 16 |
			 U64(A_H[y_ - x_][d2 >> 12 & 15]) <<  8 |
			 U64(A_H[y_ - x_][d2 >>  4 & 15])) ^
			(U64(A_L[y_ - x_][d2 >> 56 & 15]) << 56 |
			 U64(A_L[y_ - x_][d2 >> 48 & 15]) << 48 |
			 U64(A_L[y_ - x_][d2 >> 40 & 15]) << 40 |
			 U64(A_L[y_ - x_][d2 >> 32 & 15]) << 32 |
			 U64(A_L[y_ - x_][d2 >> 24 & 15]) << 24 |
			 U64(A_L[y_ - x_][d2 >> 16 & 15]) << 16 |
			 U64(A_L[y_ - x_][d2 >>  8 & 15]) <<  8 |
			 U64(A_L[y_ - x_][d2 >>  0 & 15]))) ^
			((U64(B_H[x_][y_][d1 >> 60 & 15]) << 56 |
			  U64(B_H[x_][y_][d1 >> 52 & 15]) << 48 |
			  U64(B_H[x_][y_][d1 >> 44 & 15]) << 40 |
			  U64(B_H[x_][y_][d1 >> 36 & 15]) << 32 |
			  U64(B_H[x_][y_][d1 >> 28 & 15]) << 24 |
			  U64(B_H[x_][y_][d1 >> 20 & 15]) << 16 |
			  U64(B_H[x_][y_][d1 >> 12 & 15]) <<  8 |
			  U64(B_H[x_][y_][d1 >>  4 & 15])) ^
			 (U64(B_L[x_][y_][d1 >> 56 & 15]) << 56 |
			  U64(B_L[x_][y_][d1 >> 48 & 15]) << 48 |
			  U64(B_L[x_][y_][d1 >> 40 & 15]) << 40 |
			  U64(B_L[x_][y_][d1 >> 32 & 15]) << 32 |
			  U64(B_L[x_][y_][d1 >> 24 & 15]) << 24 |
			  U64(B_L[x_][y_][d1 >> 16 & 15]) << 16 |
			  U64(B_L[x_][y_][d1 >>  8 & 15]) <<  8 |
			  U64(B_L[x_][y_][d1 >>  0 & 15])));
		y[i] ^= x[i];
	}
}
