// Copyright (c) Tony Givargis, 2024-2026

#include "g_base64.h"
#include "g_mlp.h"

struct g_mlp {
	int input;
	int output;
	int hidden;
	int layers;
	struct {
		float *w;
		float *b;
		float *a_;
		float *d_;
		float *w_;
		float *b_;
	} *net;
};

static void
mac1(float *z, const float *a, const float *b, int n, int m)
{
	for (int i=0; i<n; ++i) {
		z[i] = 0.0;
		for (int j=0; j<m; ++j) {
			z[i] += a[i * m + j] * b[j];
		}
	}
}

static void
mac2(float *z, const float *a, const float *b, int n, int m)
{
	for (int i=0; i<m; ++i) {
		z[i] = 0.0;
		for (int j=0; j<n; ++j) {
			z[i] += a[j * m + i] * b[j];
		}
	}
}

static void
mac3(float *za, const float *b, const float *c, int n, int m)
{
	for (int i=0; i<n; ++i) {
		for (int j=0; j<m; ++j) {
			za[i * m + j] += b[i] * c[j];
		}
	}
}

static void
mac4(float *za, const float *b, float s, int n)
{
	for (int i=0; i<n; ++i) {
		za[i] += b[i] * s;
	}
}

static void
add(float *za, const float *b, int n)
{
	for (int i=0; i<n; ++i) {
		za[i] += b[i];
	}
}

static void
sub(float *z, const float *a, const float *b, int n)
{
	for (int i=0; i<n; ++i) {
		z[i] = a[i] - b[i];
	}
}

static void
relu(float *za, int n)
{
	for (int i=0; i<n; ++i) {
		if (0.0 >= za[i]) {
			za[i] = 0.0;
		}
	}
}

static void
relud(float *za, const float *b, int n)
{
	for (int i=0; i<n; ++i) {
		if (0.0 >= b[i]) {
			za[i] = 0.0;
		}
	}
}

static int
size(const struct g_mlp *mlp, int l)
{
	if (0 == l) {
		return mlp->input;
	}
	if (mlp->layers == (l + 1)) {
		return mlp->output;
	}
	return mlp->hidden;
}

static void
randomize(struct g_mlp *mlp)
{
	for (int l=1; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		float a = -sqrt(6.0 / (n * m)) * 1.0;
		float b = +sqrt(6.0 / (n * m)) * 2.0;
		for (int i=0; i<(n*m); ++i) {
			mlp->net[l].w[i] = a + (rand() / (float)RAND_MAX) * b;
		}
	}
}

static void
activate(struct g_mlp *mlp, const float *x)
{
	/**
	 * a_[0] := x
	 * a_[l] := activation( w[l] * a_[l - 1] + b[l] )
	 *
	 * activation:
	 *    RELU - internal
	 *    LINEAR - output
	 */

	memcpy(mlp->net[0].a_, x, size(mlp, 0) * sizeof (mlp->net[0].a_[0]));
	for (int l=1; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		mac1(mlp->net[l].a_, mlp->net[l].w, mlp->net[l - 1].a_, n, m);
		add(mlp->net[l].a_, mlp->net[l].b, n);
		if ((l + 1) < mlp->layers) {
			relu(mlp->net[l].a_, n);
		}
	}
}

static void
backprop(struct g_mlp *mlp, const float *y)
{
	int l;

	/**
	 * d_[*] := 0
	 */

	for (l=0; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		memset(mlp->net[l].d_, 0, n * sizeof (mlp->net[l].d_[0]));
	}

	/**
	 * Quadratic Cost Function
	 *
	 * d_[L] := a_[L] − y
	 */

	l = mlp->layers - 1;
	sub(mlp->net[l].d_, mlp->net[l].a_, y, size(mlp, l));

	/**
	 * d_[l] := (w[l+1]' * d_[l+1]) ⊙ σ′(a_[l])
	 */

	while (l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		mac2(mlp->net[l - 1].d_, mlp->net[l].w, mlp->net[l].d_, n, m);
		if (1 < l) {
			relud(mlp->net[l - 1].d_, mlp->net[l - 1].a_, m);
		}
		--l;
	}

	/**
	 * b_[l] := b_[l] + d_[l]
	 * w_[l] := w_[l] + d_[l] * a_[l - 1]
	 */

	for (l=1; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		add(mlp->net[l].b_, mlp->net[l].d_, n);
		mac3(mlp->net[l].w_, mlp->net[l].d_, mlp->net[l - 1].a_, n, m);
	}
}

g_mlp_t
g_mlp_open(int input, int output, int hidden, int layers)
{
	struct g_mlp *mlp;

	assert( (1 <= input) && (1000000 >= input) );
	assert( (1 <= output) && (1000000 >= output) );
	assert( (1 <= hidden) && (1000000 >= hidden) );
	assert( (3 <= layers) && (20 >= layers) );

	// initialize

	if (!(mlp = g_malloc(sizeof (struct g_mlp)))) {
		G_TRACE("^");
		return 0;
	}
	memset(mlp, 0, sizeof (struct g_mlp));
	mlp->input = input;
	mlp->output = output;
	mlp->hidden = hidden;
	mlp->layers = layers;

	// network

	if (!(mlp->net = g_malloc(mlp->layers * sizeof (mlp->net[0])))) {
		g_mlp_close(mlp);
		G_TRACE("^");
		return 0;
	}
	memset(mlp->net, 0, mlp->layers * sizeof (mlp->net[0]));
	for (int l=0; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		mlp->net[l].a_ = g_malloc(n * sizeof (mlp->net[0].a_[0]));
		mlp->net[l].d_ = g_malloc(n * sizeof (mlp->net[0].d_[0]));
		if (!mlp->net[l].a_ || !mlp->net[l].d_) {
			g_mlp_close(mlp);
			G_TRACE("^");
			return 0;
		}
		memset(mlp->net[l].a_, 0, n * sizeof (mlp->net[0].a_[0]));
		memset(mlp->net[l].d_, 0, n * sizeof (mlp->net[0].d_[0]));
	}
	for (int l=1; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		mlp->net[l].w = g_malloc(n * m * sizeof (mlp->net[0].w[0]));
		mlp->net[l].b = g_malloc(n * 1 * sizeof (mlp->net[0].w[0]));
		mlp->net[l].w_ = g_malloc(n * m * sizeof (mlp->net[0].w[0]));
		mlp->net[l].b_ = g_malloc(n * 1 * sizeof (mlp->net[0].w[0]));
		if (!mlp->net[l].w ||
		    !mlp->net[l].b ||
		    !mlp->net[l].w_ ||
		    !mlp->net[l].b_) {
			g_mlp_close(mlp);
			G_TRACE("^");
			return 0;
		}
		memset(mlp->net[l].w, 0, n * m * sizeof (mlp->net[0].w[0]));
		memset(mlp->net[l].b, 0, n * 1 * sizeof (mlp->net[0].b[0]));
		memset(mlp->net[l].w_, 0, n * m * sizeof (mlp->net[0].w_[0]));
		memset(mlp->net[l].b_, 0, n * 1 * sizeof (mlp->net[0].b_[0]));
	}
	randomize(mlp);
	return mlp;
}

void
g_mlp_close(g_mlp_t mlp)
{
	if (mlp) {
		if (mlp->net) {
			for (int l=0; l<mlp->layers; ++l) {
				g_free(mlp->net[l].a_);
				g_free(mlp->net[l].d_);
			}
			for (int l=1; l<mlp->layers; ++l) {
				g_free(mlp->net[l].w);
				g_free(mlp->net[l].b);
				g_free(mlp->net[l].w_);
				g_free(mlp->net[l].b_);
			}
		}
		g_free(mlp->net);
		memset(mlp, 0, sizeof (struct g_mlp));
		g_free(mlp);
	}
}

const float *
g_mlp_activate(g_mlp_t mlp, const float *x)
{
	assert( mlp && x );

	activate(mlp, x);
	return mlp->net[mlp->layers - 1].a_;
}

void
g_mlp_train(g_mlp_t mlp,
	    const float *x,
	    const float *y,
	    float learning_rate,
	    int batch_size)
{
	assert( mlp && x && y );

	assert( (0.0 < learning_rate) && (1.0 >= learning_rate) );
	assert( (1 <= batch_size) && (1024 >= batch_size) );

	/**
	 * w_[*] := 0.0
	 * b_[*] := 0.0
	 */

	for (int l=1; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		memset(mlp->net[l].w_, 0, n * m * sizeof (mlp->net[0].w[0]));
		memset(mlp->net[l].b_, 0, n * 1 * sizeof (mlp->net[0].b[0]));
	}

	/**
	 * for all (x -> y):
	 *   activate()
	 *   backprop()
	 */

	for (int i=0; i<batch_size; ++i) {
		activate(mlp, x + i * mlp->input);
		backprop(mlp, y + i * mlp->output);
	}

	/**
	 * w[l] := w[l] - ( (η / batch_size) * w_[l] )
	 * b[l] := b[l] - ( (η / batch_size) * b_[l] )
	 */

	for (int l=1; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		mac4(mlp->net[l].w,
		     mlp->net[l].w_,
		     -learning_rate / batch_size,
		     n * m);
		mac4(mlp->net[l].b,
		     mlp->net[l].b_,
		     -learning_rate / batch_size,
		     n * 1);
	}
}

int
g_mlp_load(g_mlp_t mlp, const char *pathname)
{
	size_t sw, sb;
	FILE *file;

	assert( mlp && pathname && (*pathname) );

	if (!(file = fopen(pathname, "rb"))) {
		G_TRACE("unable to open file for reading");
		return -1;
	}
	for (int l=1; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		sw = (size_t)n * m * sizeof (mlp->net[0].w[0]);
		sb = (size_t)n * 1 * sizeof (mlp->net[0].b[0]);
		if ((sw != fread(mlp->net[l].w, 1, sw, file)) ||
		    (sb != fread(mlp->net[l].b, 1, sb, file))) {
			G_TRACE("unable to read file");
			return -1;
		}
	}
	fclose(file);
	return 0;
}

int
g_mlp_store(g_mlp_t mlp, const char *pathname)
{
	size_t sw, sb;
	FILE *file;

	assert( mlp && pathname && (*pathname) );

	if (!(file = fopen(pathname, "wb"))) {
		G_TRACE("unable to open file for writing");
		return -1;
	}
	for (int l=1; l<mlp->layers; ++l) {
		int n = size(mlp, l);
		int m = size(mlp, l - 1);
		sw = (size_t)n * m * sizeof (mlp->net[0].w[0]);
		sb = (size_t)n * 1 * sizeof (mlp->net[0].b[0]);
		if ((sw != fwrite(mlp->net[l].w, 1, sw, file)) ||
		    (sb != fwrite(mlp->net[l].b, 1, sb, file))) {
			G_TRACE("unable to write file");
			return -1;
		}
	}
	fclose(file);
	return 0;
}
